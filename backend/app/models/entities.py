...
class User(Base):
    ...
    discord_id: Mapped[str | None] = mapped_column(String(64))
...
class Session(Base):
    ...
    status: Mapped[str] = mapped_column(SessionStatusEnum, default="open", nullable=False)
...
class Character(Base):
    ...
    name: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    clazz: Mapped[str | None] = mapped_column(String(32))
    stats: Mapped[dict | None] = mapped_column(JSON)  # SQLite에선 TEXT 기반
...
class DiceLog(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("sessions.id"), nullable=False, index=True)
    roller_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"))
    formula: Mapped[str] = mapped_column(String(64), nullable=False)
    detail: Mapped[dict | None] = mapped_column(JSON)  # 각 눈/옵션 정보
...
class LogEntry(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("sessions.id"), nullable=False, index=True)
    category: Mapped[str] = mapped_column(String(16), default="narrative")  # narrative/system
